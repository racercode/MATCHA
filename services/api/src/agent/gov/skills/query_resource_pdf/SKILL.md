...
```txt
query_resource_pdf custom tool
-> backend
...
```txt
query_resource_pdf custom tool -> backend -> queryResourcePdfToolWrapper -> fakeData.ts summary document
```
...
- The tool name still says `pdf` for compatibility, but documents may come from PDF, Markdown, txt, html, CSV, XLSX, URL, or other sources.
