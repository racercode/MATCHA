...
# ── 資源追問（Follow-Up）─────────────────────────────────────────────
POST   /api/resource-followup              市民對媒合通知追問，Agent Q&A 模式回答
...
**Follow-Up flow** (triggered by citizen POST /api/resource-followup):
```
POST /api/resource-followup { resourceId, replyId, question }
  ...
  → initGovFollowUpSession(sessionKey = followup-{replyId})
      → 使用 GOV_FOLLOWUP_SYSTEM_PROMPT（Q&A 模式，非媒合模式）
  → runGovFollowUpQuestion(session, question)
      → agent calls query_resource_document() → reads resource docs
      → agent returns plain text answer
  → return { answer, resourceId, replyId }
```

多輪對話靠 `sessionKey = followup-{replyId}` 管理，前端重複 POST 同一個 `replyId` + 新 `question` 即可。
...
| `agent/gov/managedAgent.ts` | `initGovFollowUpSession()` 已實作（追問 Q&A 模式 session） |
| `agent/gov/pipeline.ts` | `runGovFollowUpQuestion()` 已實作（送追問、處理 tool calls、收集回答） |
| `routes/gov.ts` | `POST /api/resource-followup` 已實作（citizen 追問 API + 驗證） |
